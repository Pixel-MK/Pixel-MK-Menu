package com.pixelmkmenu.pixelmkmenu.gl;
...
public class FBO {
	...
	public void begin(int width, int height) {
		if (!supported) return;
		if (width < 1 || height < 1) throw new IllegalArgumentException("Attempted to create FBO with zero/negative size.");
		if (this.created && (width != this.frameBufferWidth || height != this.frameBufferHeight)) dispose();
		if (!this.created) {
			this.created = true;
	...
	public void dispose() {
		...
		if (this.texture != null) GL11.glDeleteTextures(this.texture.getGlTextureId());
		if (useARB) {
	...
	public void draw(double x, double y, double x2, double y2, double z, float alpha, double u, double v, double u2, double v2) {
		if (supported && this.created) {
			...
			bb.pos(x, y, z).tex(u, v2).endVertex();
			tessellator.draw();
		}
	}
}
